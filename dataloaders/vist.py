...
def LoadAnnotations(sis_file = None):
    stories = {}
    try:
...
def vcg_record_to_tokens(tokenizer: VisualCometTokenizer,
    ...
    training_instance = []
    for item in story:
        ind_training_instance = [[tokenizer.begin_img] + [tokenizer.unk_token] * num_max_boxes + [tokenizer.end_img]]
        ind_training_instance.append([tokenizer.begin_event,item['original_text'],tokenizer.end_event])
        training_instance.append(ind_training_instance)
...
class VistDataset:
    def __init__(self,
        ...
        for split in ['train', 'val', 'test']:
            split_filename = '{}_story-in-sequence.json'.format(split)
        ...
        if os.path.exists(cached_features_files) and not overwrite_cache:
            ...
            for s, split in enumerate(splits):
                ...
                split_filename = '{}_story-in-sequence.json'.format(split)
                records[split] = LoadAnnotations(os.path.join(vcg_dir, split_filename))
                ...
                for record in tqdm(records[split].items(), "Encoding Data"):
                    vcg_tokens = \
                    ...
                    tokens = []
                    for each_instance in vcg_tokens:
                        _tokens = [tokenizer.tokenize(" ".join(vt)) for vt in each_instance]
                        tokens.append(_tokens)
                        # assert len(vcg_tokens) == 4

                    if split == 'train':
                        for vt in tokens:
                            max_word = max(max_word, len(vt[1]))
                        # e_l, p_l, r_l = [len(vt[1]) for vt in tokens]
                ...
                if split == 'train':
                    print('max_image, max_event, max_place, max_inference:', max_image, max_word)
